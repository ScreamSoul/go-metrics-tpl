...
func NewRestoreMetricStorage(
	...
	if ms.IsActiveRestore && ms.restoreInit {
		ms.Load()
	}

	if ms.IsActiveRestore && ms.restoreInterval > 0 {
		go func() {
...
func (db *RestoreMetricStorage) Save() {
	...
	db.ms.logger.Info("Save metric to file")

	file, err := os.OpenFile(db.restoreFile, os.O_WRONLY|os.O_CREATE, 0666)
	if err != nil {
		db.ms.logger.Error("Error open or create file for write", zap.Error(err))
		return
	}
	defer file.Close()

	if err := json.NewEncoder(file).Encode(db.ms.List()); err != nil {
		db.ms.logger.Error("Error saving metrics to file", zap.Error(err))
	}
}

func (db *RestoreMetricStorage) Load() {

	db.ms.logger.Info("Load metric from file")

	file, err := os.OpenFile(db.restoreFile, os.O_RDONLY|os.O_CREATE, 0666)
	if err != nil {
		db.ms.logger.Error("Error open or create file for read", zap.Error(err))
		return
	}
	defer file.Close()
	...
	if err != nil || fileInfo.Size() == 0 {
		db.ms.logger.Warn("The open file has zero size or has just been created")
		return
	}

	metrics := []metrics.Metrics{}
	if err := json.NewDecoder(file).Decode(&metrics); err != nil {
		db.ms.logger.Error("Error loading metrics from file", zap.Error(err))
		return
	}
...
func (db *RestoreMetricStorage) Add(m metrics.Metrics) {
	db.ms.Add(m)
	if db.IsActiveRestore && db.restoreInterval == 0 {
		db.Save()
	}
}
